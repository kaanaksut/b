...
const EverythingView = () => {
  ...
  const renderActiveView = () => {
    switch (activeView) {
      ...
      case 'Board':
        return <p>Content of Board view</p>;
      case 'Calendar':
  ...
  return (
    <div className="everything-page">
      ...
      <div className="btn-group table-header" style={{ display: 'flex', gap: '10px' }}>
        ...
        <button
          className={`btn btn-primary ${activeView === 'Board' ? 'active' : ''}`}
          onClick={() => setActiveView('Board')}
        >
          Board
        </button>
        <button
          className={`btn btn-primary ${activeView === 'Calendar' ? 'active' : ''}`}
          onClick={() => setActiveView('Calendar')}
        >
          Calendar
        </button>
        <button
          className={`btn btn-primary ${activeView === 'Gantt' ? 'active' : ''}`}
